...
## 5. Time-log integration 的启用规则

当前专用 time-log integration 的启用条件是：

- `SUPABASE_TIME_LOG_URL` 有有效值：启用
- `SUPABASE_TIME_LOG_URL` 为空：未启用

设计要求：

- 未启用时，相关查询接口返回业务级不可用状态
- 未启用时，不能影响 `core-api` 自身启动
- `SUPABASE_TIME_LOG_BEARER_TOKEN` 只是可选凭据，不是启动强依赖
