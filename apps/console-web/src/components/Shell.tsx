...
export default function Shell({
  ...
  const [theme, setTheme] = useState<"light" | "dark">(() => {
    const saved = localStorage.getItem("asashiki.theme");
  ...
  useEffect(() => {
    ...
    localStorage.setItem("asashiki.theme", theme);
  }, [theme]);
  ...
  useEffect(() => {
    ...
    const poll = () => Health.overview().then((h) => {
      ...
      const anyErr  = !h.gateway.ok || !h.coreApi.ok || h.connectors.some(c => c.status === "err");
    ...
    <>
      <header className="topbar">
        <div className="topbar-inner">
          <div className="brand">
            Asashiki<span style={{ color: "var(--accent)" }}>.</span>
            <small>MCP CONSOLE</small>
          </div>
