...
test("console: admin login + session lifecycle", (t) => {
  const s = freshStore(t);
  s.setConsoleAdmin("asashiki", "pw123");
  assert.ok(s.verifyConsoleAdmin("asashiki", "pw123"));
  assert.ok(!s.verifyConsoleAdmin("asashiki", "nope"));
  const token = s.createConsoleSession("asashiki", 3600);
  assert.equal(s.validateConsoleSession(token), "asashiki");
